 ...
 */
class AsyncReadMemArbiter(n: Int, addrWidth: Int, dataWidth: Int) extends Module {
  val io = IO(new Bundle {
    ...
    /** Input ports */
    val in = Flipped(Vec(n, AsyncReadMemIO(addrWidth, dataWidth)))
    /** Output port */
   ...
   */
  def connect(in: AsyncReadMemIO*): AsyncReadMemIO = {
  ...
  val index = VecInit(PriorityEncoderOH(io.in.map(mem => mem.rd))).asUInt
  ...
  io.busy := busyReg
  io.chosen := chosen
  io.out <> AsyncReadMemIO.mux1H(chosen, io.in)

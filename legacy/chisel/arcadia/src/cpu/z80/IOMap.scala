 ...
 */
class IOMap(cpu: CPUIO, addrMask: Int = 0xffff) {
  /**
  ...
  class Mapping(cpu: CPUIO, r: Range) {
    ...
    def readWriteMemT(mem: MemIO)(f: UInt => UInt): Unit = {
      mem.rd := cs && !cpu.rfsh
      mem.wr := cs && cpu.iorq && cpu.wr
      mem.addr := f(addr)
      mem.din := cpu.dout
      when(cs && cpu.iorq && cpu.rd) { cpu.din := mem.dout }
    }
    ...
    def r(f: (UInt, UInt) => Bits): Unit = {
      when(cs && cpu.iorq && cpu.rd) { cpu.din := f(addr, offset) }
    }
    ...
    def w(f: (UInt, UInt, Bits) => Unit): Unit = {
      when(cs && cpu.iorq && cpu.wr) { f(addr, offset, cpu.dout) }
    }
